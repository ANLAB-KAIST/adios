...
int main(int argc, char *argv[]) {
  ...
  heap_size = ONE_GB * REGION_SIZE_GB * region_num;
  heap_start = mmap(NULL, heap_size, PROT_READ | PROT_WRITE,
             MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB | MAP_HUGE_2MB, -1, 0);

  if(heap_start == MAP_FAILED){
    printf("heap alloc failed: %d\n", errno);
    exit(-1);
  }
  fprintf(stderr, "%s, heap alloced: %p.\n", __func__, heap_start);
  ...
  int ret = 0;
  if ((ret = rdma_bind_addr(listener, (struct sockaddr *)&addr))) {
