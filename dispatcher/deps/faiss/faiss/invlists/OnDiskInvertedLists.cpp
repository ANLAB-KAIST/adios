...
extern "C" {
int __attribute__((weak)) enable_map_ddc = 0;
}
...
void OnDiskInvertedLists::prefetch_lists(const idx_t* list_nos, int n) const {
    // pf->prefetch_lists(list_nos, n); // pusnow: disable prefetch
}
...
void OnDiskInvertedLists::do_mmap() {
    ...
    FAISS_THROW_IF_NOT_FMT(
            ...
            rw_flags,
            strerror(errno));
    int map_flag = enable_map_ddc ? MAP_SHARED | MAP_DDC : MAP_SHARED;
    uint8_t* ptro =
            (uint8_t*)mmap(nullptr, totsize, prot, map_flag, fileno(f), 0);
...
InvertedLists* OnDiskInvertedListsIOHook::read_ArrayInvertedLists(
    ...
    { // do the mmap
        ...
        int map_flag__ = enable_map_ddc ? MAP_SHARED | MAP_DDC : MAP_SHARED;
