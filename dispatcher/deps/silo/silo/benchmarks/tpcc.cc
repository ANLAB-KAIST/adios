...
tpcc_worker::txn_stock_level()
{
  ...
  try {
    ...
    {
      auto s_i_ids_distinct = std::make_unique<small_unordered_map<uint, bool, 512>>();
...
protected:
  ...
  make_loaders()
  {
   ...
   virtual bench_worker *
   mkworker(uint warehouse_id_start, uint warehouse_id_end)
   {
     static std::atomic<int> workers;
    int wid = workers.fetch_add(1);
    fast_random r(wid);
    return new tpcc_worker(
              ...
              r.next(), db, open_tables, partitions,
              &barrier_a, &barrier_b,
              warehouse_id_start, warehouse_id_end);
   }
