...
allocator::Initialize(size_t ncpus, size_t maxpercore)
{
  ...
  void * const x = mmap(nullptr, g_ncpus * g_maxpercore + hugepgsize,
      PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_DDC, -1, 0); // pusnow: alloc all now
  if (x == MAP_FAILED) {
    perror("mmap");
    ALWAYS_ASSERT(false);
  }
  ...
  for (size_t i = 0; i < g_ncpus; i++) {
    ...
    g_regions[i].region_faulted = true; // pusnow: prefault not-needed
    // FaultRegion(i);
  }
...
allocator::AllocateUnmanagedWithLock(regionctx &pc, size_t nhugepgs)
{
  ...
  if (needs_mmap) {
    void * const x = mmap(mypx, hugepgsize, PROT_READ | PROT_WRITE,
        MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED | MAP_DDC, -1, 0);
