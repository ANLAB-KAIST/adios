...
def set_imgargs(options):
    ...
    if options.jvm_debug or options.jvm_suspend:
        ...
        debug_options = (
            "-agentlib:jdwp=transport=dt_socket,server=y,suspend=%s,address=5005"
    ...
    if options.trace:
        execute = (
            " ".join("--trace=%s" % name for name in options.trace) + " " + execute
        )
    ...
    if options.arch == "aarch64":
        execute = (
            "--nomount --maxnic=0 " + execute
        )  # TODO: Investigate why both virtio-blk and virtio-net does not seem to work

    options.osv_cmdline = execute
    if (
        options.kernel
        or options.hypervisor == "qemu_microvm"
        or options.arch == "aarch64"
    ):
        return

    cmdline = [
        os.path.join(osv_base, "scripts/imgedit.py"),
        "setargs",
        options.image_file,
        execute,
    ]
...
def start_osv_qemu(options):
    ...
    if options.arch == "aarch64":
        ...
        args = ["-m", options.memsize, "-smp", options.vcpus]

    if (
        not options.novnc
        and options.hypervisor != "qemu_microvm"
        and options.arch == "x86_64"
    ):
    ...
    if (
        ...
        boot_index = ""
        args += ["-kernel", options.kernel_file, "-append", options.osv_cmdline]
    else:
    ...
    if options.arch == "aarch64":
        args += [
            ...
            "cortex-a57",
            "-device",
            "virtio-blk-device,id=blk0,drive=hd0,scsi=off%s%s"
            % (boot_index, options.virtio_device_suffix),
            "-drive",
            "file=%s,if=none,id=hd0,%s" % (options.image_file, aio),
        ]
        ...
        args += [
            ...
            "q35",
            "-drive",
            "file=%s,if=none,id=hd0,media=disk,%s" % (options.image_file, aio),
            "-device",
            "ide-hd,drive=hd0,id=idehd0,bus=ide.0",
        ]
    elif options.scsi:
        args += [
            "-device",
            "virtio-scsi-pci,id=scsi0%s" % options.virtio_device_suffix,
            "-drive",
            "file=%s,if=none,id=hd0,media=disk,%s" % (options.image_file, aio),
            "-device",
            "scsi-hd,bus=scsi0.0,drive=hd0,scsi-id=1,lun=0%s" % boot_index,
        ]
    elif options.ide:
        args += ["-hda", options.image_file]
    else:
        args += [
            "-device",
            "virtio-blk-pci,id=blk0,drive=hd0,scsi=off%s%s"
            % (boot_index, options.virtio_device_suffix),
            "-drive",
            "file=%s,if=none,id=hd0,%s" % (options.image_file, aio),
        ]
    ...
    if options.cloud_init_image:
        args += [
            "-device",
            "virtio-blk-pci,id=blk1,bootindex=1,drive=hd1,scsi=off%s"
            % options.virtio_device_suffix,
            "-drive",
            "file=%s,if=none,id=hd1" % (options.cloud_init_image),
        ]
    ...
    for idx in range(int(options.nics)):
        ...
        if options.networking:
            if options.tap:
                args += [
                    "-netdev",
                    "tap,id=hn%d,ifname=%s,script=no,downscript=no"
                    % (idx, options.tap),
                ]
            elif options.vhost:
                args += [
                    "-netdev",
                    "tap,id=hn%d,script=%s,downscript=%s,vhost=on"
                    % (
                        idx,
                        os.path.join(osv_base, "scripts/qemu-ifup.sh"),
                        os.path.join(osv_base, "scripts/qemu-ifdown.sh"),
                    ),
                ]
            else:
                ...
                args += [
                    "-netdev",
                    "bridge,id=hn%d,br=%s,helper=%s"
                    % (idx, options.bridge, bridge_helper),
                ]
            ...
            args += [
                "-netdev",
                "user,id=un%d,net=192.168.122.0/24,host=192.168.122.1%s"
                % (idx, forward_options),
            ]
        ...
        if not options.vmxnet3:
            net_device_options_str = (
                net_device_options_str + options.virtio_device_suffix
            )
    ...
    if options.virtio_fs_dir:
        try:
            ...
            if e.errno == errno.ENOENT:
                print(
                    "virtiofsd binary not found. Please install the qemu-system-x86 package that comes with it (>= 4.2) and is in path."
                )
            else:
                print(
                    'OS error({0}): "{1}" while running virtiofsd {2}'.format(
                        e.errno, e.strerror, " ".join(args)
                    )
                )
    ...
    try:
        ...
        if e.errno == errno.ENOENT:
            print(
                "'%s' binary not found. Please install the qemu-system-x86 package."
                % qemu_path
            )
        else:
            print(
                'OS error({0}): "{1}" while running qemu-system-x86_64 {2}'.format(
                    e.errno, e.strerror, " ".join(args)
                )
            )
    finally:
...
def start_osv_xen(options):
    ...
    if not options.novnc:
        vncoptions = re.match(
            "^(?P<vncaddr>[^:]*):?(?P<vncdisplay>[0-9]*$)", options.vnc
        )
    ...
    try:
        ...
        subprocess.call(
            [
                ...
                "vmdk",
                options.image_file,
                "build/%s/osv.vmdk" % options.opt_path,
            ]
        )
        # Launch vmware
...
def choose_hypervisor(external_networking, arch):
    if os.path.exists("/dev/kvm") and arch == "x86_64":
        return "kvm"
    if (
        os.path.exists("/proc/xen/capabilities")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--debug", action="store_true", help="start debug version"
    )
    parser.add_argument(
        "-r", "--release", action="store_true", help="start release version"
    )
    parser.add_argument(
        "-w",
        "--wait",
    ...
    parser.add_argument("-t", "--tap", action="store", help="tap interface name")
    parser.add_argument(
        "-m",
        "--memsize",
        action="store",
    ...
    parser.add_argument(
        "-c", "--vcpus", action="store", default="4", help="specify number of vcpus"
    )
    parser.add_argument(
        "-e",
        "--execute",
    ...
    parser.add_argument(
        "-g", "--graphics", action="store_true", help="Enable graphics mode."
    )
    parser.add_argument(
        "-V",
        "--verbose",
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="do not run, just print the command line"
    )
    parser.add_argument(
        "--jvm-debug", action="store_true", help="start JVM with a debugger server"
    )
    parser.add_argument(
        "--jvm-suspend",
        action="store_true",
    ...
    parser.add_argument("--mac", action="store", help="set MAC address for NIC")
    parser.add_argument(
        "--vnc", action="store", default=":1", help="specify vnc port number"
    )
    parser.add_argument(
        "--api",
        action="store_true",
    ...
    parser.add_argument(
        "--trace", default=[], action="append", help="enable tracepoints"
    )
    parser.add_argument(
        "--trace-backtrace",
        action="store_true",
    ...
    parser.add_argument("--novnc", action="store_true", help="disable vnc")
    parser.add_argument("--nogdb", action="store_true", help="disable gdb")
    parser.add_argument(
        "--gdb", action="store", default="1234", help="specify gdb port number"
    )
    parser.add_argument(
        "--script", action="store", help="XEN define configuration script for vif"
    )
    parser.add_argument(
        "--cloud-init-image",
        action="store",
    ...
    parser.add_argument(
        "--virtio",
        action="store",
        choices=["legacy", "transitional", "modern"],
    ...
    parser.add_argument("--virtio-fs-tag", action="store", help="virtio-fs device tag")
    parser.add_argument(
        "--virtio-fs-dir",
        action="store",
    ...
    cmdargs.opt_path = (
        "debug" if cmdargs.debug else "release" if cmdargs.release else "last"
    )
    cmdargs.image_file = os.path.abspath(
        cmdargs.image or os.path.join(osv_base, "build/%s/usr.img" % cmdargs.opt_path)
    )
    if cmdargs.arch == "aarch64":
        cmdargs.kernel_file = os.path.join(
            osv_base, "build/%s/loader.img" % cmdargs.opt_path
        )
    else:
        cmdargs.kernel_file = os.path.abspath(
            cmdargs.kernel_path
            or os.path.join(osv_base, "build/%s/kernel.elf" % cmdargs.opt_path)
        )
    ...
    if cmdargs.cloud_init_image:
        ...
        if not os.path.exists(cmdargs.cloud_init_image):
            raise Exception(
                "Cloud-init image %s does not exist." % cmdargs.cloud_init_image
            )

    if cmdargs.virtio_fs_dir and not os.path.exists(cmdargs.virtio_fs_dir):
        raise Exception(
            "Directory %s to be exposed through virtio-fs does not exist."
            % cmdargs.virtio_fs_dir
        )
