...
static int uverbs_ioctl(struct device *dev, unsigned long request, void *data) {
    auto *prv = reinterpret_cast<struct uverbs_priv *>(dev->private_data);
    return prv->drv->make_ioctl_request(request, data);
}
...
int uverbs::make_ioctl_request(unsigned long request, void *data) {
    std::unique_ptr<struct uverbs_hdr> hdr(new struct uverbs_hdr);
    std::memset(hdr.get(), 0, sizeof(*hdr.get()));
    hdr->type = VIRTIO_UVERBS_DEV_IOCTL;
    hdr->prot.ioctl.request = request;

    struct ib_uverbs_ioctl_hdr *ioctl_hdr_user =
        (struct ib_uverbs_ioctl_hdr *)data;

    assert(ioctl_hdr_user->length > 0);

    std::unique_ptr<char[]> ioctl_cmd(new char[ioctl_hdr_user->length]);
    memset(ioctl_cmd.get(), 0, ioctl_hdr_user->length);
    memcpy(ioctl_cmd.get(), data, ioctl_hdr_user->length);

    struct ib_uverbs_ioctl_hdr *ioctl_hdr =
        (struct ib_uverbs_ioctl_hdr *)ioctl_cmd.get();

    int err = virtio_uverbs_translate_ioctl(ioctl_hdr);
    if (err) return -1;

#if 0
    printf("[before] ");
    for (unsigned short ih = 0; ih < ioctl_hdr->length; ++ih) {
        printf("%d ", ioctl_cmd[ih]);
    }
    printf("\n");
#endif
    ...
    queue->init_sg();
    queue->add_out_sg(hdr.get(), sizeof(*hdr.get()));
    queue->add_out_sg(ioctl_cmd.get(), ioctl_hdr->length);
    queue->add_in_sg(resp.get(), sizeof(*resp.get()));
    queue->add_buf_wait(hdr.get());
    queue->kick();
