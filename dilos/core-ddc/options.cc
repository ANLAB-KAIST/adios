...
void parse_cmdline(
    ...
    if (::options::option_value_exists(options_values, "ddc_disable")) {
        ddc_disable = std::stoi(
            ::options::extract_option_value(options_values, "ddc_disable")) > 0;
    } else {
        ddc_disable = false;
    }
}
