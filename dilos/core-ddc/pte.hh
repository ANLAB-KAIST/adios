...
constexpr size_t protect_pte_mask = (1ULL << protect_pte_offset) - 1;
constexpr size_t protect_pte_addr_mask = ~protect_pte_mask;
...
template <int N>
inline pt_element<N> protected_pte_with_addr(hw_ptep<N> &_, uintptr_t page) {
    auto pte = pt_element<N>(page);
...
template <int N>
inline unsigned get_protect_data(pt_element<N> element) {
    return element.raw() >> protect_pte_offset;
}

template <int N>
inline uintptr_t get_protect_addr(pt_element<N> element) {
    return element.raw() & protect_pte_addr_mask;
}
