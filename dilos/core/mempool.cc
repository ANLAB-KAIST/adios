...
static void* malloc_large(size_t size, size_t alignment, bool block = true, bool contiguous = true)
{
    ...
    if (size >= (mmu::huge_page_size << 3) && !contiguous) {
...
constexpr u64 ddc_phymem = 0xffff800100000000ULL;
...
void free_initial_memory_range(void* addr, size_t size)
{
    ...
    if((u64)addr == ddc_phymem){
        // we will use only last segment
        assert(phys_list_next < max_phys_list);
        assert(size >= DDC_PHYMEM_ADDITIONAL_SIZE);

        debug_early_u64("DDC_PHYMEM_ADDITIONAL_SIZE: ", DDC_PHYMEM_ADDITIONAL_SIZE);

        phys_list[phys_list_next].first = (void*)((u64)addr  + DDC_PHYMEM_ADDITIONAL_SIZE);
        phys_list[phys_list_next].second =  size - DDC_PHYMEM_ADDITIONAL_SIZE;
        size = DDC_PHYMEM_ADDITIONAL_SIZE;
        ++phys_list_next;
    }
...
std::vector<std::pair<void *, size_t>> list_phys(){
    ...
    for(size_t i =0; i < memory::phys_list_next; ++i){
        result.emplace_back(memory::phys_list[i].first, memory::phys_list[i].second);
    }
