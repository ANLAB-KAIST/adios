...
interrupt_descriptor_table::interrupt_descriptor_table()
{
    ...
    for (unsigned i = 160; i < 256; ++i) { // from this disable ist
        add_entry(i, 0, reinterpret_cast<void (*)()>(interrupt_entry + (i - 32) * 16));
    }
}
...
unsigned interrupt_descriptor_table::register_interrupt_handler_legacy(
        std::function<bool ()> pre_eoi,
        std::function<void ()> eoi,
        std::function<void ()> post_eoi)
{
...
unsigned interrupt_descriptor_table::register_handler_legacy(std::function<void ()> post_eoi)
{
    return register_interrupt_handler_legacy([] { return true; }, [] { processor::apic->eoi(); }, post_eoi);
}
