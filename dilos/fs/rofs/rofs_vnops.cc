...
static int rofs_read_without_cache(struct vnode *vnode, struct file *fp, struct uio *uio, int ioflag)
{
    ...
    void *buf = memory::alloc_phys_contiguous_aligned(BSIZE * 512, 8, true);

    // printf("[rofs] rofs_read [%d], inode: %d, [%d -> %d] at %d of %d bytes\n",
    ...
    while (block_count > 0){
        ...
        error = rofs_read_blocks(device, block, block_count_this, buf);
        if (error) {
            kprintf("[rofs_read] Error reading data\n");
        ...
        // for(int cnt = 0; uio->uio_iovcnt; ++cnt){
        //     printf("uio_iov[%d]: %p %lx\n", cnt, uio->uio_iov[cnt].iov_base, uio->uio_iov[cnt].iov_len);
        // }


        uint64_t read_amt_this =  std::min<uint64_t>(read_amt, (block_count_this*BSIZE) - offset);
        // printf("read_amt_this: %ld\n",read_amt_this);
        rv = uiomove(buf + offset, read_amt_this, uio);
        read_amt -= read_amt_this;
        offset = 0; // only first block
    }
