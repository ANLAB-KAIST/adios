...
void premain()
{
    ...
    for (auto init = inittab.start; init < inittab.start + (inittab.count > INIT_BAR ? INIT_BAR : inittab.count); ++init) {
        (*init)();
    }
...
void premain2()
{
    ...
    for (auto init = inittab.start + (inittab.count > INIT_BAR ? INIT_BAR : inittab.count); init < inittab.start + inittab.count; ++init) {
        (*init)();
    }
}
