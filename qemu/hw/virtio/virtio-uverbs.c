...
#include "../../../virtio-uverbs-trans/virtio-uverbs/ioctl.h"
...
static unsigned int do_process_uverbs_ioctl(VirtQueueElement *elem, struct uverbs_resp *resp, int uverbs_fd, unsigned long request)
{
    int ret = -1;


    struct ib_uverbs_ioctl_hdr *ioctl_hdr =
        (struct ib_uverbs_ioctl_hdr *) elem->out_sg[1].iov_base;

    ret = ioctl_translate(ioctl_hdr);

    if (ret){
        printf("do_process_uverbs_ioctl: fail (%d)\n", ret);
        abort();
    }
    errno=0;
    ret = ioctl(uverbs_fd, request, elem->out_sg[1].iov_base);


    resp->status = VIRTIO_UVERBS_S_OK;
    resp->prot.ioctl.ret = ret;
    return 0;
}
...
static void do_process_uverbs(VirtIOUverbs *vuverbs)
{
    ...
    while (!virtio_queue_empty(vuverbs->uverbsq))
    {
        ...
        case VIRTIO_UVERBS_DEV_MUNMAP:
            ...
            break;
        case VIRTIO_UVERBS_DEV_IOCTL:
            len = do_process_uverbs_ioctl(elem, &resp, vuverbs->uverbs_fd, hdr.prot.ioctl.request);
            break;
        default:
